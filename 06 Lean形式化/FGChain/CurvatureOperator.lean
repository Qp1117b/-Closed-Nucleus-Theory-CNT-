...
[环节7] 耦合常数-固有时流速不确定性关系（δ_v → v_τ → Δu·Δv_τ ≥ C/2）
   ↓
[环节8] 耦合常数算符 û = ln r̂（CQM 海森堡对 [û, p̂_u] = iC，C 为相变量子）
```

## 数学内容

- **曲率算符**：自伴、实本征值（角亏实值）、与振荡模的正性衔接。
- **CQM 海森堡对**：[û, p̂_u] = iC——以相变量子 C = ξ'(1)/ξ(1) 取代 ħ 的角色
...
/-- **耦合常数算符数据**：û = ln r̂（r 为总耦合强度），
    共轭动量 p̂_u = v̂_τ / C（固有时流速 / 相变量子），
    CQM 海森堡代数 [û, p̂_u] = iC（C = 相变量子 ξ'(1)/ξ(1) 取代 ħ 的角色）。 -/
structure CouplingOperator where
  /-- 相变量子 C = ξ'(1)/ξ(1) ≈ 0.0230957（无量纲，GL(5) 固定层级） -/
  C : ℝ
  /-- 相变量子为正 -/
  C_pos : 0 < C
...
/-- **环节7 ⟷ 环节8 衔接定理（CQM 不确定性关系的链路实例化）**：
    ...
    耦合常数-固有时流速不确定性关系为
    (Δr/⟨r⟩)·Δv_τ ≥ C/2，
    其中 C 为相变量子（《CQM_核心_集成理论》§3.4 的链路表述）。
