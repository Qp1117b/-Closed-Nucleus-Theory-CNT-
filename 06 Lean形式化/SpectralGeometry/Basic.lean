...
CQM 的谱几何：相变量子 C、Mathieu 临界值 λ_c、Sierra-CQM 耦谱定理。
...
## 推导链
A₄ 嘉当矩阵 → 本征值 → Mathieu 参数 → λ_c → 相变量子 C → 耦级 𝔠₁ → κ → G_N
...
/-- [DEFINITION] 相变量子 C：CQM 中最基本的无量纲常数。
...
/-- 相变量子 C 远小于 A₄ 的最小本征值 λ₁ = (3-√5)/2 ≈ 0.382 -/
...
/-- 耦合空间中的相变量子 C 与耦合速度 c 的关系（声明）：
    在非禁闭区域，c ≈ C（耦合速度趋于相变量子）。
    这是耦合空间离散性的直接体现。

    当前严格证明：相变量子 C > 0（见 `spectralQuantum_pos`）。
