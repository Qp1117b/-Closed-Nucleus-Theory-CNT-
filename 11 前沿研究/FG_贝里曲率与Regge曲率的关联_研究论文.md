...
**(4) 非阿贝尔推广**：Wilczek–Zee 非阿贝尔贝里曲率 $F = d\mathcal{A} + \mathcal{A}\wedge\mathcal{A}$【文献确立】[3] 与本框架伴丛曲率 $F_\ell = d\mathcal{A}_\ell + \mathcal{A}_\ell\wedge\mathcal{A}_\ell$（`FG_纤维丛理论.md` §3）公式同型；Wilson 环 $W = \mathcal{P}\exp(i\oint\mathcal{A})$ 与本框架和乐 $W_v^{(\ell)} = \exp(i\delta_v^{(\ell)}\hat{T}_\ell)$（§3.1）同型。
...
| 表述 | 位置 | 性质 |
|:---|:---|:---|
| $W_v^{(\ell)} = \exp(i\delta_v^{(\ell)}\hat{T}_\ell)$，和乐平庸化 = 稳定构型 | `FG_纤维丛理论.md` §3.1 | 框架结构（数学上为 Wilson 环） |
| 一个联络生成两种曲率：Regge 角亏 + 伴丛曲率 $F_\ell = d\mathcal{A}_\ell + \mathcal{A}_\ell\wedge\mathcal{A}_\ell$ | `FG_纤维丛理论.md` §3 | 框架结构 |
| "内禀角亏 $\delta_{\text{intrinsic}}$ 无内部来源——当前只能引入外部 DFT Berry 曲率" | `CQM_超导_FG层级同步算符体系.md` §1.2 | 【CQM 构造假设】（外部输入依赖） |
| "DFT Berry 曲率是电子波函数在动量空间的几何相位——在 CQM 语言中它是伴丛和乐的某种唯象近似" | 同上 §9.3 | 【CQM 构造假设】 |
| 黎曼–西格尔相角 $\theta(t)$ = "同步基态的 Berry 相位" | `CQM_核心_因果网络同步理论.md` §3.7 | 【CQM 构造假设】 |
| 和乐 = 共形场论单值性（monodromy），$h = \delta_v\hat{T}/2\pi$ | `FG_纤维丛理论.md` §9.4 | 【CQM 构造假设】（映射构造） |
...
1. **"密度泛函理论贝里曲率 = 伴丛和乐的唯象近似"（§9.3）**：文献分析显示两者测的是**不同空间的几何数据**——密度泛函理论贝里曲率是动量空间（布里渊区）能带几何[7][18]；$\delta_{\text{intrinsic}}$ 是实空间分子几何偏离理想杂化几何的球面角亏（`CQM_超导_FG层级同步算符体系.md` §1.2 诊断）。"动量空间带几何 ↔ 实空间组织几何"的等同性是构造假设，文献既未建立也未否定。该假设在数学范畴上自洽（同为曲率—和乐结构），但跨空间映射本身无先例。
...
FG"一个联络生成两种曲率"（`FG_纤维丛理论.md` §3）不是冗余设计，而是度量—响应分工（存在论层级分析见 §5.6.3）：
...
框架原文的定位支持进一步的层级区分（`三种引力存在论前提.md` §3.3、§7、§8）：
...
2. 若问题 1 成立，动量空间外部输入（密度泛函理论贝里曲率[18]）是应被构型空间内部来源**替代**，还是与后者**互补**？——即 §9.3"伴丛和乐唯象近似"定位（`CQM_超导_FG层级同步算符体系.md`）的两种可能结局，何者成立？
